...
function ShopHome() {
   ...
   function hdlNavigateToListPage(getCurrItem , section) {
  ...
  dispatch(addToCart({
   ...
   useEffect(() => {
    const timer = setInterval(() => {
      setCurrSlide(prevSlide => (prevSlide + 1) % slides.length);
    }, 4000);
   ...
   console.log(productList , "pro pileyr");
        ...
        <div className="flex flex-col min-h-screen">
         <div className="relative w-full h-[600px] overflow-hidden">
            {
               ...
               className={`${index === currSlide ? "opacity-100" : "opacity-0"} absolute top-0 left-0 w-full h-full transition-opacity duration-1000`} 
                />))}

                <Button variant="outline" size="icon"
                onClick={() => {
                  setCurrSlide(prevSlide=> (prevSlide -1 + slides.length) % slides.length)
               }}
         ...
         <section className='py-12 bg-gray-50'>
            <div className='container mx-auto px-4'>
               ...
               <div className='gap-4 mt-5 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5'>
                  {
                     categoriesWithIcon.map(categoryItem => <Card onClick={() => hdlNavigateToListPage(categoryItem , 'category')}
         ...
         <section className='py-12 bg-gray-50'>
            <div className='container mx-auto px-4'>
               ...
               <div className='gap-4 mt-5 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6'>
                  {
                     brandsWithIcon.map(brandItem => <Card  onClick={() => hdlNavigateToListPage(brandItem , 'brand')}
